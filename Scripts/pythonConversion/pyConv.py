...
if uploadedFile:
    ...
    if True:
        if matSamp:
            ...
            if (df["Status"].eq(dct["userTerm"])).any():
                inpt = st.text_input(f'Would you like to replace {df["Status"]} with {dct["userTerm"]}? ')

                if inpt.lower() == "yes":
                    df["Status"] = str(dct["userTerm"])

            else:
                ask = st.text_input(f'Whould you like to replace {df["Status"].unique()}?')

                if ask.lower() == "yes":
                    replace = st.text_input(f'What would you like to replace {df["Status"].unique()} with? ')
                    if replace:
                        uniq_vals = []
                        replace_split = []
                        uniq_vals = df["Status"].unique()
                        d = {}
                        replace_split = replace.split(",")
                        for i in range(len(uniq_vals)):
                            globals()['object%s' % i] = df['Status'].eq(uniq_vals[i])
                            replacement = replace_split[i]
                            df['Status'][('object%s' % i) == True] = replacement
            dct.to_csv('MST_dict.csv')
        if verLoc:
            df = remove_rcna(df)
            locality_cols= []
        ...
        if datamelt:
            ...
            cols = st.text_input("Input the columns you would like to melt seperated by commas")
            if cols:
                dataCols = cols.split (",")
        ...
        if True:
            ...
            if matSamp:
                ...
                f'<a href="data:file/csv" download = {dct} >Download csv file</a>'
